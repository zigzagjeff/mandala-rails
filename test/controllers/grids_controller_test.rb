...
class GridsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "show subscribes to the mandala's event stream" do
    ...
    get mandala_grid_path(@mandala, child)

    assert_response :success
    assert_select "turbo-cable-stream-source[signed-stream-name=?]",
      Turbo::StreamsChannel.signed_stream_name(@mandala)
  end
  ...
  test "breadcrumb at depth 2 links the intermediate tile to its child grid" do
    ...
    get mandala_grid_path(@mandala, grandchild)

    assert_response :success
    assert_select ".breadcrumb-current", text: "Seattle"
    assert_select ".breadcrumb a[href=?]", mandala_grid_path(@mandala, child), text: @tile.title
  ...
  test "show cannot reach another user's grid" do
    ...
    get mandala_grid_path(@mandala, other_grid)
