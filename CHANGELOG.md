...
- The sign-in page's "Need an account? Sign up" link was a bare `<p>` outside `.form-actions`, so it wrapped underneath the submit button and read as a rendering bug rather than a call to action. That is the first screen a stranger sees, and a first outside visitor stalled on it. Moved into the form flow above the button as prose ("**New here?** Sign up to create an account."), matching `fizzy-main`'s `sessions/new.html.erb`. No CSS change — the layout it needed already existed
- The seeded Start Here mandala told every new user "There is no privacy. Everyone can see everyone's Mandala." That has been false since every controller began scoping through `Current.user`, and it was the first thing a new account read. Replaced with the truth, and the EU hosting facts (Hetzner Nuremberg, Scaleway Paris) promoted out of "What's next?" — which was listing shipped accounts and login as though still upcoming. A regression test now asserts the privacy claim
- Start Here's walkthrough still instructed the reader to click the `▶` corner button this branch deletes, so merging would have shipped onboarding copy describing a control that no longer exists. Rewritten against the three zones the partial actually renders, including why a blank tile has no ring (R6: naming a tile reveals the next affordance)

### Security
- Rails 8.1.3 → 8.1.3.1 for **CVE-2026-66066** / GHSA-xr9x-r78c-5hrm — possible arbitrary file read and remote code execution in Active Storage variant processing. The advisory published 2026-08-02 and dependabot's last run was 2026-07-24, so `bin/check`'s bundler-audit stage caught it first. `Gemfile` already permitted the patch (`~> 8.1.3`); only the lock moves
