...
### Added
- First system tests (`test/system/`) — `ApplicationSystemTestCase` (headless Chrome via selenium) with `sign_in_as`, `wait_for_cable_connection`, and `rename_tile` helpers; a regression test that the drill arrow survives a tile rename (the CHANGELOG 0.2.0 fix, previously untested) and a live-update test that an out-of-band write surfaces in the browser via the #68 broadcast; CI installs Chrome and runs `test:system`, uploading screenshots from failures (part of #91)
